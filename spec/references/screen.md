...
by default, which is how a screen ships all three states unanswered while looking
finished.

The default is not a hypothetical. Measured on one repository: 70 screens used the kit's
table, 27 overrode its empty text, 43 did not. Before designing an empty state, check
whether the screen next to yours answered it — and copy the answer rather than inventing
a fourth phrasing.
